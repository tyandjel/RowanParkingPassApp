...
public class DatabaseHandlerDrivers extends DatabaseHandlerBase {
    ...
    private static final String KEY_FIRST_NAME_STR = "first_name_str";
    private static final String KEY_LAST_NAME_STR = "last_name_str";
    ...
    public void addVistor(String fname, String lname, String street, String city, String state, String zip) {
        ...
        values.put(KEY_FIRST_NAME_STR, fname); // Name
        values.put(KEY_LAST_NAME_STR,lname);// Last name
    ...
    public void updateUser(String uid, String fname, String lname, String street, String city, String state, String zip) {
        ...
        ContentValues values = new ContentValues();
        values.put(KEY_FIRST_NAME_STR, fname); // First Name
        values.put(KEY_LAST_NAME_STR, lname);//Last Name
        values.put(KEY_STREET_STR, street); // Street
        values.put(KEY_CITY_STR, city); // City
        values.put(KEY_STATE_STR, state); // State
    ...
    public ArrayList<Driver> getVisitors() {
        ...
        while (!cursor.isAfterLast()) {
            ...
            user.put(KEY_FIRST_NAME_STR, cursor.getString(2));
            user.put(KEY_LAST_NAME_STR, cursor.getString(3));
            user.put(KEY_STREET_STR, cursor.getString(4));
            user.put(KEY_CITY_STR, cursor.getString(5));
            user.put(KEY_STATE_STR, cursor.getString(6));
            user.put(KEY_ZIP_INT, cursor.getString(7));
            rows.add(new Driver(user.get(KEY_DRIVER_ID), user.get(KEY_FIRST_NAME_STR), user.get(KEY_LAST_NAME_STR), user.get(KEY_STREET_STR), user.get(KEY_CITY_STR), user.get(KEY_STATE_STR), user.get(KEY_ZIP_INT)));
            user.clear();
            cursor.moveToNext();
        }
        cursor.close();
